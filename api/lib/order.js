// Shared order-creation + CJ fulfillment logic, used by the simulated checkout
// path and the Stripe webhook listener.
...
// Enrich raw cart items with their current DB product record (price/stock/etc).
export async function fetchDetailedItems(items) {
  const detailed = [];
  for (const it of (items || [])) {
    const { data: product, error } = await supabase
      .from('products')
      .select('id, title, price, images, stock, cj_product_id')
      .eq('id', it.product_id)
      .single();
...
export function totalsFromDetailed(detailed) {
  const subtotal = +detailed
    .reduce((s, d) => s + Number(d.product.price) * d.quantity, 0)
    .toFixed(2);
...
// Create the order + order_items, decrement stock, and submit to CJ.
export async function createOrderFromCart({ user, customerEmail, shippingAddress, items }) {
  ...
  for (const d of detailed) {
    itemRows.push({
      ...
      image: d.product.images?.[0]?.url || null,
    });
  ...
  const cj = await fulfillOrderWithCJ({
    orderId: order.id,
    customerEmail,
    shippingAddress,
    items: cjItems,
